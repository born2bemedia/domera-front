import type { BriefFormSchema, RequestFormSchema } from '../model/schemas';
...
export async function submitBriefForm(data: BriefFormSchema): Promise<void> {
  const res = await fetch('/api/forms', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ formType: 'brief', data }),
  });
